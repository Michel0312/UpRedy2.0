...
async def seven(client: Client, message: Message):
	...
	if len(lista) == 2:
		...
		if not "." in j:
			p = shutil.make_archive(j, format = "zip", root_dir=g)
			await h.edit("⚜️𝑫𝒊𝒗𝒊𝒅𝒊𝒆𝒏𝒅𝒐 𝒆𝒏 𝒑𝒂𝒓𝒕𝒆𝒔⚜️")
...
async def up(client: Client, message: Message):	
	...
	try:
		...
		elif Configs[username]["m"] == "tesisld":
			print("tesisld")
			await tesisld_api(path,user_id,msg,username)
		else:
		...
		if filename.split(".")[-1]=="txt":
			...
			if "medisur.sld.cu" in urls[0]:
				...
				async with aiohttp.ClientSession(connector=connector) as session:
					...
					for url in urls:
						try:
							id = url.split("download/")[1].split("/")[0]
							du = f"{host}author/deleteSubmission/{id}"
							async with session.get(du) as r:
								if r.status==302 or r.status==200:
									p+=1
									await ms.edit(f"**💢 Borrando enlaces {p}/{t} . Espere por favor 💢**")
						except:pass
					if p==t:
						await ms.edit("✅ **Todos los archivos han sido borrados** ✅")
					else:
						await ms.edit(f"**❗️ Solo fueron borrados {p} de {t} archivos**")
					os.unlink(path)
					return
				...
				async with aiohttp.ClientSession(connector=connector) as session:
					...
					async with session.post(host+"index.php?P=UserLogin", data=payload,ssl=False) as e:
						print(222)
					for url in urls:
						try:
							url_id = url.split(">")[1]
							d_id = url.split("Id=")[1].split(">")[0]
							payload = {}
							payload["F_Title"] = ""
							...
							query = {"Imagen":"application/octet-stream",**payload}
	...
	if "youtu.be/" in message.text or "twitch.tv/" in message.text or "youtube.com/" in message.text or "xvideos.com" in message.text or "xnxx.com" in message.text:
		...
		async with aiohttp.ClientSession() as session:
			async with session.get(url) as r:
				...
				f.close()
				file = f"{j}{filename}"
				await msg.edit("⬇️𝑫𝒆𝒔𝒄𝒂𝒓𝒈𝒂 𝒆𝒙𝒊𝒕𝒐𝒔𝒂⬇️")
				if procesos != 0:
					procesos -= 1
				else:pass
				await msg.edit("✅𝑨𝒓𝒄𝒉𝒊𝒗𝒐 𝒈𝒖𝒂𝒓𝒅𝒂𝒅𝒐✅")
...
async def downloadmessage_progres(chunk,filesize,filename,start,message):
		...
		msg+= f"⬇️ 𝙳𝚘𝚠𝚗𝚕𝚘𝚊𝚍𝚒𝚗𝚐 ⬇️: {sizeof_fmt(chunk)} of {sizeof_fmt(filesize)} \n\n"	
		global seg
		if seg != localtime().tm_sec:
...
async def mediafiredownload(chunk,total,filename,start,message):
	...
	msg+= f"⬇️ 𝙳𝚘𝚠𝚗𝚕𝚘𝚊𝚍𝚒𝚗𝚐 ⬇️: {sizeof_fmt(chunk)} of {sizeof_fmt(total)}\n\n"
	global seg
	if seg != localtime().tm_sec:
		try: await message.edit(msg)
		except:pass
...
async def upspe_api(file,usid,msg,username):
	try:
		zipssize=Configs[username]['z']*1024*1024
		filename = file.split("/")[-1]
		host = "http://catalogobibliotecaupspe.sld.cu/"
		...
		async with aiohttp.ClientSession(connector=connector) as session:
			...
			async with session.get(url ,ssl=False) as resp:
				...
				if u==url:
					...
					await msg.edit("🟢 Conección establecida ... 🟢")
					sleep(5)
					print(22)
					links = []
					if mode=='n':
...
async def callback_data(bot,callback):
	...
	if data[0]=="clear":
		...
		if data[1]=="medisur":
			...
			await call_back.edit_text("✅ UPSPE API Client Upload Activate ✅")
		elif data[1]=="tesisld":
